...
from mon_anomaly import anomaly_exceptions
...
class ZookeeperStateTracker(object):
    ...
    def __init__(self, url, topic, finished_queue, max_lag):
        ...
        self.topic_path = '/consumers/mon-anomaly/%s' % topic

        self.lock_retry_time = 15  # number of seconds to wait for retrying for the lock
        self.lock_path = '/locks/mon-anomaly/%s' % topic
    ...
    def run(self):
        ...
        """
        if not self.has_lock:
            raise anomaly_exceptions.AnomalyException('Attempt to begin run without Zookeeper Lock')
